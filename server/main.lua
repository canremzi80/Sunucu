...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "muhittin" then
			for _, v in pairs(Config.Shops.Muhittin.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "tekno" then
			for _, v in pairs(Config.Shops.Tekno.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
...
AddEventHandler("suku:SellItemToPlayer",function(source, type, item, count)
    ...
    if type == "item_standard" then
        ...
        if targetItem.limit == -1 or ((targetItem.count + count) <= targetItem.limit) then
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Ödedin: '..count.." "..list[i].label })
					else
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Envanterinde yer yok!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Ödedin: '..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Yeterli Paran Yok!' })
						end
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Zaten bu silahtan var!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bu silaha sahipsin!' })
        end
	end
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				if targetWeapon then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bu mermiden alamassın!' })
				end
            end
        end
    end
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.get('money') >= Config.LicensePrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Silah lisansı aldın.' })
